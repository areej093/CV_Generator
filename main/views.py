    ...
    if request.method == 'POST':
        try:
            ...
            if cv_id:
                ...
                cv.full_name = data.get('personalInfo', {}).get('name', 'My Name')
                cv.email = data.get('personalInfo', {}).get('email', '')
                cv.phone = data.get('personalInfo', {}).get('phone', '')
                cv.address = data.get('personalInfo', {}).get('address', '')
                cv.professional_title = data.get('personalInfo', {}).get('title', '')
                cv.summary = data.get('personalInfo', {}).get('summary', '')
                cv.linkedin_url = data.get('personalInfo', {}).get('linkedin', '')
...
def download_pdf(request, cv_id):
    ...
    try:
        ...
        # Generate QR Code - use LinkedIn URL if provided, else fallback to vCard
        if cv.linkedin_url:
            qr_data = cv.linkedin_url
        else:
                ...
                <div class="cv-container">
                    ...
                    <div class="footer">
                        <div style="border-top: 1px solid #8b7355; margin-top: 20px; padding-top: 15px; display: flex; justify-content: space-between; align-items: center;">
                            ...
                            <div style="text-align: right;">
                                <img src="data:image/png;base64,{qr_base64}" style="width: 90px; height: 90px; border: 1px solid #8b7355;">
...
def manage_trainings(request):
    ...
    if request.method == 'POST':
        title = request.POST.get('title')
        provider = request.POST.get('provider_name')
        description = request.POST.get('description')
